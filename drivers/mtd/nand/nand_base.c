						  ...
						  int busw, int *maf_id)
{
	...
	*maf_id = chip->read_byte(mtd);
	...
	manufactor_id = *maf_id;
	master_id = (manufactor_id << 8) |dev_id; 
	...
	if (!type->pagesize) {
		...
		if ( ((master_id == 0xecd7) && (chip->cellinfo == 0xd5)) 
		 	|| ((master_id == 0xecd5) && (chip->cellinfo == 0x94)) ){
			/* Calc pagesize */
			mtd->writesize = 2048 << (extid & 0x3);
			extid >>= 2;
			
			/* Calc oobsize */
			mtd->oobsize = (8 << (extid & 0x3)) * (mtd->writesize >> 9);
