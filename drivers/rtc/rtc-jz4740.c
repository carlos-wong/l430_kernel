...
static inline uint32_t resource_size(struct resource *mem)
{
	return mem->end - mem->start + 1;
}
...
static int jz4740_rtc_set_mmss(struct device *dev, unsigned long secs)
{
	...
	printk("set_mmss : %d size:%d size:%d¥n",secs,sizeof(uint32_t),sizeof(secs));
	//if ((uint32_t)secs != secs)
	//	return -EINVAL;

	jz4740_rtc_reg_write(rtc, JZ_REG_RTC_SEC, secs);
	jz4740_rtc_ctrl_set_bits(rtc,0,0);
	

	return 0;
}

static int jz4740_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	unsigned long time;

	rtc_tm_to_time(tm,&time);

	return jz4740_rtc_set_mmss(dev,time);
}
...
//	rtc->mem = request_mem_region(rtc->mem->start, rtc->mem->end - rtc->mem->start + 1,
//					pdev->name);
